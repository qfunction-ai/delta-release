  ...
  const fetch = useCallback(async (overrideUrl?: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const msg = await extractApiError(res, options?.errorMessage || 'Failed to load data');
        if (!controller.signal.aborted) {
          setError(msg);
        }
      }
      ...
      if (!controller.signal.aborted) {
        setError(options?.connectionErrorMessage || options?.errorMessage || 'Connection error');
      }
    } finally {
      if (!controller.signal.aborted) {
        setLoading(false);
